...
#let typsidian(
  ...
  set page(
    ...
    columns: 1,
    fill: colors.bg,
    margin: (top: 6.5em, right: 5.5em, bottom: 6em, left: 5.5em),
    header: context {
    ...
    footer: context {
      let _footer-text
      if footer-text != none { _footer-text = footer-text }
      else { _footer-text = author }
  ...
  if show-index {
    ...
    [
      ...
      #show figure: it => { align(left, it.body) }
...
#let make-title(show-outline: true, show-underline: true, show-author: false, justify: "center", outline-depth: 2) = context {
  let colors = _colors.get()
