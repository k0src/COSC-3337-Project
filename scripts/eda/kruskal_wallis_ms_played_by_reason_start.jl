...
function print_kw(display_name, H, df, p, reasons, mean_ranks, groups)
  p_str = p < 0.0001 ? "< 0.0001" : @sprintf("%.4f", p)
  println("$display_name")
  println("  H = $(@sprintf("%.4f", H))  |  df = $df  |  p = $p_str")
  println()
  println("  Group summary:")
  @printf "    %-16s  %8s  %12s  %12s  %12s\n" "reason_start" "n" "mean_rank" "median_min" "mean_min"
  for (reason, mr, g) in zip(reasons, mean_ranks, groups)
    @printf "    %-16s  %8d  %12.2f  %12.4f  %12.4f\n" reason length(g) mr median(g) mean(g)
  end
  println()
end
...
function main()
  ...
  for (username, display_name) in NAMES
    ...
    for reason in reasons
      vals = Float64.(filter(r -> String(r.reason_start) == reason, user_df).ms_played) ./ 60000.0
      length(vals) < 3 && continue
      push!(groups, vals)
      push!(valid_reasons, reason)
    end

    length(groups) < 2 && continue

    H, df_val, p, mean_ranks = kruskal_wallis(groups)
    print_kw(display_name, H, df_val, p, valid_reasons, mean_ranks, groups)

    dunn_results = dunn_test(groups, valid_reasons)
    ...
    all_data[display_name] = Dict{String,Any}(
      "kruskal_wallis" => Dict{String,Any}(
        ...
        "groups" => [
          ...
          for (reason, g, mr) in zip(valid_reasons, groups, mean_ranks)
        ],
      ),
      "dunn_bonferroni" => [
        Dict{String,Any}(
  ...
  save_json("kruskal_wallis_ms_played_by_reason_start_2025.json", all_data)
end
