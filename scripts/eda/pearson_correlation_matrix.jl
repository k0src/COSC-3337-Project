...
function plot_corr_matrix(C, label, fname)
  ...
  ax = Axis(fig[1, 1],
    title="Pearson Correlation Matrix — $label",
    xticks=(1:p, VARS),
    yticks=(1:p, VARS),
...
function main()
  ...
  for (username, display_name) in NAMES
    ...
    plot_corr_matrix(C, display_name, "pearson_corr_$(username).png")
  ...
  plot_corr_matrix(C_pool, "All Users (Pooled)", "pearson_corr_pooled.png")
  ...
  save_json("pearson_correlation_matrix.json", all_data)
end
