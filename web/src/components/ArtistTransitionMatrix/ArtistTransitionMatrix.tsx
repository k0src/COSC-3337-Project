...
import styles from "./ArtistTransitionMatrix.module.css";
...
interface ArtistTransitionMatrixProps {
...
export default function ArtistTransitionMatrix({ mainUser }: ArtistTransitionMatrixProps) {
  const [tooltip, setTooltip] = useState<{
    from: string; to: string; prob: number; count: number; x: number; y: number;
  } | null>(null);

  const mainUserColor = USERS.find((u) => u.displayName === mainUser)?.color;
  const { data, loading } = useData<AllData>("artist_transition/2025");

  const handleMove = useCallback((from: string, to: string, prob: number, count: number, e: React.MouseEvent) => {
  ...
  const { artists, matrix, counts } = userData;
    ...
    <div className={styles.section}>
      ...
      <div className={styles.card}>
        <p className={styles.description}>
          Probability of playing one artist after another (row to column)
        </p>
        <svg viewBox={`0 0 ${svgW} ${svgH}`} width="100%" style={{ display: "block" }}>
          {/* Column labels — rotated, anchored at bottom of label area */}
          ...
          {/* Cells */}
          {artists.map((fromArtist, ri) =>
            artists.map((toArtist, ci) => {
              ...
              return (
                <rect
                  ...
                  onMouseMove={(e) => handleMove(fromArtist, toArtist, val, count, e)}
